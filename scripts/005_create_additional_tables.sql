...
CREATE TABLE IF NOT EXISTS public.reports (
  ...
  content_type VARCHAR(20) NOT NULL CHECK (content_type IN ('Article', 'Comment')),
...
CREATE POLICY "reports_create_own" ON public.reports
...
CREATE POLICY "urls_owner_all" ON public.urls
  FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "urls_public_read" ON public.urls
  FOR SELECT USING (true); -- Allow public access for URL resolution
