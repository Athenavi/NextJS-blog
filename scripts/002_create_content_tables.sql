...
CREATE TABLE IF NOT EXISTS public.article_content (
  article_id INTEGER PRIMARY KEY REFERENCES public.articles(id) ON DELETE CASCADE,
...
CREATE POLICY "article_content_public_read" ON public.article_content
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "article_content_author_all" ON public.article_content
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.articles a 
...
CREATE POLICY "comments_author_update_delete" ON public.comments
