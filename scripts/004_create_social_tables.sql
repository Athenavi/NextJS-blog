...
CREATE TABLE IF NOT EXISTS public.user_subscriptions (
  ...
  subscribed_user_id UUID REFERENCES public.users(id) ON DELETE CASCADE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(subscriber_id, subscribed_user_id)
);
...
CREATE POLICY "user_subscriptions_public_read" ON public.user_subscriptions
  FOR SELECT USING (true); -- Allow reading who follows whom

-- RLS policies for category_subscriptions
CREATE POLICY "category_subscriptions_owner_all" ON public.category_subscriptions
  FOR ALL USING (auth.uid() = subscriber_id);
