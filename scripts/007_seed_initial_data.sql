...
WITH role_permission_mapping AS (
  ...
  WHERE 
    (r.name = 'admin') OR
    (r.name = 'moderator' AND p.code IN ('article.read', 'comment.read', 'comment.moderate', 'moderate.content')) OR
    (r.name = 'author' AND p.code IN ('article.create', 'article.read', 'article.update', 'article.publish', 'comment.create', 'comment.read', 'media.upload')) OR
    (r.name = 'user' AND p.code IN ('article.read', 'comment.create', 'comment.read', 'user.read'))
)
